...
setwd("l:/baihao/UHI/2025/all_pixel_watermask0")
output <- "./figs_full"
log_path <- "log.txt"
...
# ADM2_CODE_list
which(ADM2_CODE_list == 3787)

for (i in 1:dim(ADM2_CODE_list)[1]) { #nrow(df)
...
#save
write_lines(pass_list,'./all_model_results/pass_list_allpixel_watermask0_vif.txt')
write_csv(result_all,'./all_model_results/result_all_allpixel_watermask0_vif.csv')
write_csv(result_pars,'./all_model_results/result_pars_allpixel_watermask0_vif.csv')
...
ggsave('./figs/figs4_watermask0_vif.jpg',p1,dpi = 300, height = 7, width = 5)
ggsave('./figs/figs4_watermask0_vif.pdf',p1,dpi = 300, height = 7, width = 5)
