...
setwd("K:/baihao/UHI/2025/all_pixel_watermask0")
output <- "./figs_full"
log_path <- "log.txt"

mat.name=paste("all_pixel_watermask0",".mat",sep="")
...
get_model_rrp <- function(m){
  ...
  m_rmse_r2 <- c(m_rmse,m_r2,m_p_value)
...
for (i in 1:dim(ADM2_CODE_list)) { #nrow(df)
  ...
  model.list <- list(
    model2_day = lm(LST_Day ~ albedo + built_height + building_count + srtm_dem + Building_height_variance + f_bio + f_fossil + f_metals + f_minerals,
                    data = pixel_data_adm2),
    model3_day = lm(LST_Day ~ tree_canopy_cover + built_height + building_count + srtm_dem + Building_height_variance + f_bio + f_fossil + f_metals + f_minerals,
                    data = pixel_data_adm2),
    model4_day = lm(LST_Day ~ vegetation_cover + built_height + building_count + srtm_dem + Building_height_variance + f_bio + f_fossil + f_metals + f_minerals, 
                    data = pixel_data_adm2),
    model5_day = lm(LST_Day ~ NDVI + built_height + building_count + srtm_dem + Building_height_variance + f_bio + f_fossil + f_metals + f_minerals,
                    data = pixel_data_adm2),
    model2_night = lm(LST_Night ~ albedo + built_height + building_count + srtm_dem + Building_height_variance + f_bio + f_fossil + f_metals + f_minerals,
                      data = pixel_data_adm2),
    model3_night = lm(LST_Night ~ tree_canopy_cover + built_height + building_count + srtm_dem + Building_height_variance + f_bio + f_fossil + f_metals + f_minerals,
                      data = pixel_data_adm2),
    model4_night = lm(LST_Night ~ vegetation_cover + built_height + building_count + srtm_dem + Building_height_variance + f_bio + f_fossil + f_metals + f_minerals, 
                      data = pixel_data_adm2),
    model5_night = lm(LST_Night ~ NDVI + built_height + building_count + srtm_dem + Building_height_variance + f_bio + f_fossil + f_metals + f_minerals,
                      data = pixel_data_adm2)
  )
  ...
  models_rr$metrics <- c('rmse','r2','p_value')
...
#save
write_lines(pass_list,'./all_model_results/pass_list_allpixel_watermask0.txt')
write_csv(result_all,'./all_model_results/result_all_allpixel_watermask0.csv')
write_csv(result_pars,'./all_model_results/result_pars_allpixel_watermask0.csv')
...
ggsave('./figs/figs4_watermask0.jpg',p1,dpi = 300, height = 7, width = 5)
ggsave('./figs/figs4_watermask0.pdf',p1,dpi = 300, height = 7, width = 5)
